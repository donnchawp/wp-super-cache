<div class="wpsc-card" id="lockdown">
	<h2><?php _e( 'Lock Down:', 'wp-super-cache' ); ?> <?php echo $wp_lock_down == '0' ? '<span class="dashicons dashicons-unlock"></span> ' . __( 'Disabled', 'wp-super-cache' ) : '<span class="dashicons dashicons-lock"></span> ' . __( 'Enabled', 'wp-super-cache' ); ?></h2>
	...
	<div class="wpsc-code-block"><code>if( defined( 'WPLOCKDOWN' ) && constant( 'WPLOCKDOWN' ) ) {
...
submit_button( esc_attr( $new_lockdown_desc . ' ' . __( 'Lock Down', 'wp-super-cache' ) ) );
wp_nonce_field( 'wp-cache' );
echo '</form>';

?><?php
if( $cache_enabled == true && $super_cache_enabled == true ) {
?><div class="wpsc-card" id="direct">
	<h2><?php _e( 'Directly Cached Files', 'wp-super-cache' ); ?></h2><?php
...
if( !is_writeable_ACLSafe( ABSPATH ) ) {
	$readonly = 'READONLY';
	?><div class="notice notice-warning inline"><p><strong><?php _e( 'Warning!', 'wp-super-cache' ); ?></strong> <?php printf( __( 'You must make %s writable to enable this feature. As this is a security risk, please make it read-only after your page is generated.', 'wp-super-cache' ), ABSPATH ); ?></p></div><?php
} else {
	$abspath_stat = stat(ABSPATH . '/');
	$abspath_mode = decoct( $abspath_stat[ 'mode' ] & 0777 );
	if ( substr( $abspath_mode, -2 ) == '77' ) {
		?><div class="notice notice-warning inline"><p><strong><?php _e( 'Warning!', 'wp-super-cache' ); ?></strong> <?php printf( __( '%s is writable. Please make it readonly after your page is generated as this is a security risk.', 'wp-super-cache' ), ABSPATH ); ?></p></div><?php
	}
}
...
if( is_array( $cached_direct_pages ) ) {
	...
	if( $out != '' ) {
		?><table class="wp-list-table widefat striped"><tr><th><?php _e( 'Existing direct page', 'wp-super-cache' ); ?></th><th><?php _e( 'Delete cached file', 'wp-super-cache' ); ?></th></tr><?php
		echo "$out</table>";
	}
}
...
if ( 'READONLY' !== $readonly ) {
	...
	submit_button( esc_attr__( 'Update Direct Pages', 'wp-super-cache' ) );
}
wp_nonce_field( 'wp-cache' );
echo '</form>';
?>
